<?php
 ...
 $admin = $bdd->recupererAdmin($session);
  if (isset($_POST['objet']) &&$_POST['contenu']&& $_POST['destinataire']){
    $c = $_POST['contenu'];
    $o = $_POST['objet'];
    if($_POST['destinataire'] == "tous"){
      $agents = $bdd->recupererListeAgents();
      foreach($agents as $agent){
        $message = new Contact(0, $admin->getIdAdministrateur(), $agent->getIdAgent(), $o, $c,date('y-m-d'), 0);
      $bdd->ajouterMessage($message);
      }
      $clients = $bdd->recupererListeClients();
      foreach($clients as $client){
        $message = new Contact(0, $admin->getIdAdministrateur(), $client->getIdClient(), $o, $c,date('y-m-d'), 0);
      $bdd->ajouterMessage($message);
      }
    }else if ($_POST['destinataire'] == "clients"){
      $clients = $bdd->recupererListeClients();
      foreach($clients as $client){
        $message = new Contact(0, $admin->getIdAdministrateur(), $client->getIdClient(), $o, $c,date('y-m-d'), 0);
      $bdd->ajouterMessage($message);
      }
    }else if($_POST['destinataire'] == "agents"){
      $agents = $bdd->recupererListeAgents();
      foreach($agents as $agent){
        $message = new Contact(0, $admin->getIdAdministrateur(), $agent->getIdAgent(), $o, $c,date('y-m-d'), 0);
      $bdd->ajouterMessage($message);
      ...
      $message = new Contact(0, $admin->getIdAdministrateur(), $destinataire, $o, $c,date('y-m-d'), 0);
      $bdd->ajouterMessage($message);
    }
    $envoyer = 1;
  }
...
<body>
  <!--[if lte IE 9]>
		...
		<?php
       headerAdmin($session);
        $messages=$bdd->recupererListeAdminMessageRecu($admin->getIdAdministrateur());
        $messagesE=$bdd->recupererListeAdminMessageEnvoye($admin->getIdAdministrateur());
        $agents = $bdd->recupererListeAgents();
			?>
  ...
  <button id="newMessageBtn" class="btn">Envoyer un message</button>
  ...
  <div id="messagePanel" class="panel">
    <h2 style="text-align: center;font-size: 24px;margin-bottom: 20px;" >Envoyer un message<span class="close" id="closeBtn">&times;</span></h2>
    ...
    <div class="col-md-6">
        <div class="form-group">
          ...
          <select id="destinataire" name="destinataire">
            <option value="tous">Client et Agents</option>
            <option value="agents">Agents</option>
            <option value="clients">Client</option>
            <?php
            foreach($agents as $agent){
              echo ' <option value="'.$agent->getIdAgent().'">'.$agent->parent->getNom().' '.$agent->parent->getPrenom().'</option>';
            }
            ?>
        </select>
      </div>  
    ...
    <div class="inbox">
      ...
      if(count($messages) == 0){
        ...
        echo '<a class="link" href="messagerieAdmin.php?idMessage='.$message->getIdMessage().'">';
        echo '</a>';
      ...
      echo "</div>";
       ...
       echo "</div>";
        ...
        echo '</div>';
        echo '</div>';
        $bdd->lireMessage($message->getIdMessage(), $admin->getIdAdministrateur());
        }else{
          echo '<div class="contenu" >';
...
<script>
  ...
  recu.addEventListener('click', function (){
      window.location.href = "messagerieAdmin.php";
  });
  envoye.addEventListener('click', function (){
      window.location.href = "messagerieAdmin.php?envoye=";
  });
