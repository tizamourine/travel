...
<style>
	...
	<body>
		...
		<div class="search-input">
			<form action="listeSejour.php" method="get">
		...
		<?php
		if (isset($_GET['s'])){$sejours = $bdd->searchSejour($_GET['s']);
		if($_GET['s']==''){$sejours = $bdd->listeSejour();}}
		if (count($sejours) == 0) echo "<p> aucun sejour de la sorte. </p>";
		foreach($sejours as $sejour){
			echo <<<"EOT"
			<div class="ho">
			<p>
			EOT; echo $sejour->parent->getDestination(); echo "</p>";
		    echo "<p>".$sejour->parent->getNbrPlaces();

			echo '</p><button class="modifier" type="button" value="'.$sejour->getIdSejour().'"';echo '> modifier le sejour</button>
			 	<button class="supprimer" type="button" value="'.$sejour->getIdSejour().'"';echo'> supprimer le sejour</button></div>';
		}
		?>
		</div>
		...
		<div class="filter-input">
			<form>
				<label for="filtrer"> Liste des sejours </label><br>
				<a href=<?php echo '"sejour.php"';?>> Ajouter un sejour </a>
	...
	<form>
		...
		<script>
			...
			function mHotel(i){
				window.location.href = "modSejour.php?idSejour="+m[i].value;
			}
			function sHotel(i){
				form.setAttribute("action", "supprSejour.php?idSejour="+s[i].value);
