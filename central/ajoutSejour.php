...
if (isset($_POST['description']) && isset($_POST['transport'])&& isset($_POST['destination'])&& isset($_POST['mdp'])&& isset($_POST['iteneraire'])&& isset($_POST['prixDefault'])&& isset($_POST['nbrPlaces'])&&isset($_POST['equipement'])&& isset($_POST['prixAdulte'])&&isset($_POST['prixEnfnts'])&& isset($_POST['prixBebe'])&&isset($_POST['heureDepart'])&& isset($_POST['dateD'])&&isset($_POST['dateR'])&& isset($_POST['nbrJours'])&& isset($_POST['hotel'])&& isset($_POST['CSRFToken'])){
  ...
  $sejour = new Sejour(
    ...
    if($bdd->verifierUtilisateur($admin->parent->parent->getEmail(), $mdp)[0] == 4){
        $bdd->insertSejour($sejour);
        if ($total_files > 0){
        $idSejour = $bdd->VoyageOrganises($sejour);
        $upload_directory = "images/Sejour/".$idSejour."/";
        ...
        echo "<script>window.location.href='sejour.php?q=1'</script>";
    }else{
        echo "<script>window.location.href='sejour.php?q=2'</script>";
    }
}else{
    echo "<script>window.location.href='sejour.php?q=4'</script>";
}


if ($token != ""){
  if($bdd->verifierToken($token, $session) == 0){
    echo "<script>window.location.href='sejour.php?q=3'</script>";
  }                     
}
?>
</body>
