...
<html class="no-js"  lang="en">
	...
	input[type="submit"] {
		...
		<section id="home" class="about-us" style="z-index: 1; height: 100%;">
				...
				if (count($Reservations) == 0) echo "<p> aucune reservation de la sorte. </p>";
							foreach ($Reservations as $Reservation){
								...
								if ($Reservation->getIdHotel() == 0){
                                    ...
                                    reservation($bdd->recupererVoyageImage($Reservation->getIdVoyage())[0], "Voyage Organise", $voyage->getDestination(), "Payé", $Reservation->getDate(), $Reservation->getProgression(), $Reservation->getIdReservation());
                                    echo '<button class="annuler" type="button" value="'.$Reservation->getIdReservation().'" style="position:absolute;right:3px;bottom:3px;background-color:#008CBA;height: 40px;width: 90px;padding: 10px;text-align:center;color:white;">Consulter</button></div>';
                                }else{
                                    $chambre = $bdd->recupererChambre($Reservation->getIdHotel());
                                    $hotel = $bdd->recupererHotel($chambre->getIdHotel());
                                    reservation($bdd->recupererChambreImage($Reservation->getIdHotel())[0], "hotel", $hotel->getNomHotel()." Chambre: ".$chambre->getNumeroCH(), "Payé", $Reservation->getDate(), $Reservation->getProgression(), $Reservation->getIdReservation());
                                    echo '<button class="annuler" type="button" value="'.$Reservation->getIdReservation().'" style="position:absolute;right:3px;bottom:3px;background-color:#008CBA;height: 40px;width: 90px;padding: 10px;text-align:center;color:white;">Consulter</button></div>';
                                }
							}
						}
			...
			function sHotel(i){
				window.location.href="consulterReservation.php?idReservation="+s[i].value;
			}
