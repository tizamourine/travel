 ...
 $envoyer = 0;
  if (isset($_POST['objet']) &&$_POST['contenu']&& $_POST['destinataire']){
    ...
    $o = $_POST['objet'];
      ...
      $message = new Contact(0, $agent->getIdAgent(), $destinataire, $o, $c,date('y-m-d'), 0);
...
<body>
  <!--[if lte IE 9]>
		...
		<?php
       headerAgent($session);
        $messages=$bdd->recupererListeMessageRecu($agent->getIdAgent());
        $messagesE=$bdd->recupererListeMessageEnvoye($agent->getIdAgent());
        $clients = $bdd->recupererListeAgentClients($agent->getIdAgent());
			?>
  ...
  <div id="messagePanel" class="panel">
        ...
        <div class="form-group">
          ...
          <select id="destinataire" name="destinataire">
            ...
            foreach($clients as $client){
              echo ' <option value="'.$client->getIdClient().'">'.$client->parent->getNom().' '.$client->parent->getPrenom().'</option>';
            }
            ?>
        </select>
      </div>  
    ...
    <div class="inbox">
      ...
      if(count($messages) == 0){
        ...
        echo '<a class="link" href="messagerieAgent.php?idMessage='.$message->getIdMessage().'">';
        echo '</a>';
      ...
      echo "</div>";
       ...
       echo "</div>";
        ...
        echo '</div>';
        echo '</div>';
        $bdd->lireMessage($message->getIdMessage(), $agent->getIdAgent());
        }else{
          echo '<div class="contenu" >';
...
<script>
  ...
  recu.addEventListener('click', function (){
      window.location.href = "messagerieAgent.php";
  });
  envoye.addEventListener('click', function (){
      window.location.href = "messagerieAgent.php?envoye=";
  });
