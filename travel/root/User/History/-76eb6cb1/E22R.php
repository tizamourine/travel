<?php
    ...
    if (isset($_GET['idSejour']
		...
		if(isset($_POST['informationSup'])&&isset($_POST['nbrAdulte'])&&isset($_POST['prixChambre'])&& isset($_POST['nbrBebe'])&& isset($_POST['nbrEnfnts'])){
			...
			$prix = $Sejour->parent->calculerPrix($nbrEnfnts, $nbrAdulte, $nbrBebe, $prixChambre-1000);
  ...
  button[type="submit"]:hover {
	...
	<body style="position:absolute;width:100%;height:100%;">
		...
		<section id="home" class="about-us" style="z-index: 1; height: 100%;">
			<div class="settings-container" style="top: 80px;">
			   ...
			   <div class="reserver">
				<form method="POST" >
					...
					<fieldset>
					<legend>Chambre d'hotel</legend>
					<select name="prixChambre" >
					<?php
					$chambres = $bdd->listeChambre($Sejour->getIdHotel());
					foreach($chambres as $chambre){
						echo '<option value="'.$chambre->getPrix().'">'.$chambre->getTypeChambre().'</option>';
					}
					?>
					</select>
					</fieldset>
			...
			let commentaire = getElementById('commentaire');
			let vide =0;
			...
			if(vide == 1){
				...
				commentaire.value="Remplissez tous les champs";
			}else
				form.submit();
			});
		</script>
